...
<head>
    ...
    <title>Кредиторы - FillFunds</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-gray-500 ml-1 md:ml-2">Кредиторы</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
            <div class="flex items-center space-x-3 mb-4 md:mb-0">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Кредиторы</h1>
                    <p class="text-gray-600 mt-1">Управление списком кредиторов и задолженностей</p>
                </div>
            </div>
            
            <!-- Add Creditor Button -->
            <a 
                href="/categories/creditors/create" 
                class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-600 to-pink-600 text-white font-medium rounded-xl hover:from-red-700 hover:to-pink-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
            >
        ...
        <?php if (empty($creditors)): ?>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-12 text-center card-hover">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-3">Нет добавленных кредиторов</h3>
                <p class="text-gray-600 mb-6 max-w-md mx-auto">Начните с добавления первого кредитора для учета задолженностей</p>
                <a 
                    href="/categories/creditors/create" 
                    class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-600 to-pink-600 text-white font-medium rounded-xl hover:from-red-700 hover:to-pink-700 transition-all duration-200"
                >
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    </svg>
                    Добавить первого кредитора
                </a>
            </div>
        <?php else: ?>
            <!-- Creditors Grid -->
                ...
                <?php foreach ($creditors as $creditor): ?>
                    <div class="bg-white rounded-xl shadow-lg p-6 card-hover">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center space-x-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-red-100 to-pink-100 rounded-lg flex items-center justify-center">
                                    <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                    </svg>
                                </div>
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-900"><?= htmlspecialchars($creditor['name']) ?></h3>
                                    <p class="text-sm text-gray-500">Кредитор</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <button 
                                    onclick="editCreditor(<?= $creditor['id'] ?>)" 
                                ...
                                <button 
                                    onclick="deleteCreditor(<?= $creditor['id'] ?>, '<?= addslashes(htmlspecialchars($creditor['name'])) ?>')" 
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-500">ID кредитора:</span>
                                <span class="font-mono text-gray-900">#<?= $creditor['id'] ?></span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <!-- Summary Card -->
            <div class="mt-8 bg-gradient-to-br from-red-50 to-pink-50 rounded-xl p-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-lg font-semibold text-gray-900">Всего кредиторов: <?= count($creditors) ?></p>
                        <p class="text-sm text-gray-600">Управляйте списком ваших кредиторов для точного учета задолженностей</p>
                    </div>
                </div>
            </div>
        <?php endif; ?>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-2xl bg-white">
            <div class="mt-3 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900" id="modal-title">Удалить кредитора</h3>
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500" id="modal-message">
                        Вы уверены, что хотите удалить этого кредитора? Это действие нельзя отменить.
                    </p>
                </div>
    ...
    <script type="module">
        ...
        let creditorToDelete = null;

        // Edit creditor function
        window.editCreditor = function(id) {
            window.location.href = `/categories/creditors/edit?id=${id}`;
        };

        // Delete creditor function
        window.deleteCreditor = function(id, name) {
            creditorToDelete = id;
            modalMessage.textContent = `Вы уверены, что хотите удалить кредитора "${name}"? Это действие нельзя отменить.`;
        ...
        deleteConfirm.addEventListener('click', function() {
            if (creditorToDelete) {
                ...
                form.method = 'POST';
                form.action = '/categories/creditors/delete';
                
                const idInput = document.createElement('input');
                idInput.type = 'hidden';
                idInput.name = 'id';
                idInput.value = creditorToDelete;
